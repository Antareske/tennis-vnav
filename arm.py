...
class ZP10SGripperAdapter:
    ...
    def open(self) -> None:
        from zl.zp10s.uart_control import release

        release(self._zp10s)
        self._status = "open"

    def close(self) -> None:
        from zl.zp10s.uart_control import grab
...
class STS3215GripperAdapter:
    ...
    def open(self) -> None:
        from sts3215 import release

        release(self._servo)

    def close(self) -> None:
        from sts3215 import grab
...
def create_gripper(
    ...
    if driver == "zp10s":
        from zl.zp10s.uart_control import ZP10S
